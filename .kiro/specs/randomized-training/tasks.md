...
- [ ] 1. Extend domain model and GraphQL schema
  - [ ] 1.1 Add `isRandomized`, `randomizedWordCount`, and `units` optional fields to the `Training` interface in `backend/src/model/domain/Training.ts`
...
- [ ] 2. Implement randomized training creation in TrainingService
  - [ ] 2.1 Modify `createTraining` in `backend/src/services/training-service.ts` to accept `isRandomized` and `randomizedWordCount` parameters
  ...
  - [ ] 2.2 Update `Mutation.createTraining.ts` Lambda resolver to pass `isRandomized` and `randomizedWordCount` from input to the service
    - _Requirements: 1.1, 4.3, 4.4_

  - [ ]* 2.3 Write property test: Randomized training creation stores configuration correctly (Property 1)
    - **Property 1: Randomized training creation stores configuration correctly**
    - **Validates: Requirements 1.1, 1.2**

  - [ ]* 2.4 Write property test: Word count validation and capping (Property 2)
    - **Property 2: Word count validation and capping**
    - **Validates: Requirements 1.4, 1.5**

  - [ ]* 2.5 Write property test: Static training backward compatibility (Property 5)
    - **Property 5: Static training backward compatibility**
    - **Validates: Requirements 3.1, 3.3**

  - [ ]* 2.6 Write unit tests for randomized creation edge cases
...
- [ ] 4. Implement dynamic word selection on startTraining
  - [ ] 4.1 Modify `startTraining` in `backend/src/services/training-service.ts` to branch on `training.isRandomized`
  ...
  - [ ]* 4.2 Write property test: Dynamic word selection produces a correct subset (Property 3)
    - **Property 3: Dynamic word selection produces a correct subset**
    - **Validates: Requirements 2.1, 2.2, 2.3, 2.6**

  - [ ]* 4.3 Write property test: Deleted vocabulary lists are skipped (Property 4)
    - **Property 4: Deleted vocabulary lists are skipped during word selection**
    - **Validates: Requirements 2.4**

  - [ ]* 4.4 Write property test: Multiple choice options from dynamically selected words (Property 6)
    - **Property 6: Multiple choice options generated from dynamically selected words**
    - **Validates: Requirements 7.1**

  - [ ]* 4.5 Write unit tests for startTraining edge cases
...
- [ ] 5. Implement dual-path submitAnswer
  - [ ] 5.1 Modify `submitAnswer` in `backend/src/services/training-service.ts` to resolve words from `execution.words` when training is randomized, or from `training.words` when static
  ...
  - [ ]* 5.2 Write unit tests for submitAnswer dual-path
...
- [ ] 7. Update frontend training provider and GraphQL queries
  - [ ] 7.1 Update `createTraining` in `frontend/src/lib/providers/training_provider.dart` to accept optional `isRandomized` and `randomizedWordCount` parameters and include them in the mutation input
    - _Requirements: 5.3, 5.4_

  - [ ] 7.2 Update `loadTrainings` and `getTraining` GraphQL queries in `training_provider.dart` to fetch `isRandomized` and `randomizedWordCount` fields
    - _Requirements: 6.1, 6.2_

  - [ ] 7.3 Update `startTraining` GraphQL mutation in `training_provider.dart` to fetch `execution.words` field in the response
    - _Requirements: 2.6, 4.5_

- [ ] 8. Update frontend training creation UI
  - [ ] 8.1 Add a toggle for randomized mode on the training creation screen
...
- [ ] 9. Update frontend training list and detail views
  - [ ] 9.1 Show a visual indicator (e.g., shuffle icon) for randomized trainings in the training list
    - _Requirements: 6.1_

  - [ ] 9.2 Update training detail view to show randomized word count and vocabulary list count instead of a fixed word list for randomized trainings
