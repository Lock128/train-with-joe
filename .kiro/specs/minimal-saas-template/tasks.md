# Implementation Plan: Minimal SaaS Template

## Overview

This implementation plan breaks down the minimal SaaS template into discrete coding tasks. The template provides a production-ready foundation for Flutter-based SaaS applications with AWS backend infrastructure, including authentication, data storage, GraphQL API, payment processing (Stripe + App Store + Play Store), AI content enhancement, and Angular landing pages.
...
- [x] 1. Initialize project structure and configuration files
  - **Copy from nexus-share**: Root directory structure, package.json, tsconfig.json, vitest.config.ts, eslint.config.mjs, .prettierrc, .husky/, cdk.json
  ...
  - Copy package.json from nexus-share/backend and remove social media dependencies (keep aws-cdk-lib, vitest, esbuild, eslint, prettier, husky, stripe, fast-check)
  - Copy tsconfig.json from nexus-share/backend
  - Copy vitest.config.ts from nexus-share/backend
  - Copy eslint.config.mjs from nexus-share/backend (flat config format)
  - Copy .prettierrc from nexus-share/backend
  - Copy .husky/ directory from nexus-share for pre-commit hooks
  - Copy cdk.json from nexus-share/backend and simplify context
  - Copy .gitignore from nexus-share
...
- [x] 2. Implement base CDK stack with core infrastructure
  - [x] 2.1 Create BaseStack with Cognito user pool configuration
    - **Copy from nexus-share**: backend/lib/base-stack.ts (Cognito configuration section)
    - Copy Cognito user pool setup from nexus-share BaseStack
  ...
  - [x] 2.2 Add DynamoDB tables to BaseStack
    - **Copy from nexus-share**: backend/lib/base-stack.ts (DynamoDB table definitions)
    - Copy Users table definition from nexus-share BaseStack (simplify by removing social-specific fields)
    - Copy or create Subscriptions table with id partition key and userId GSI
    - Keep billing mode, encryption, and removal policy settings from nexus-share
    - Export table names and ARNs
    - _Requirements: 1.2, 4.1, 4.7_
  
  - [x] 2.3 Add S3 bucket for application assets
    - **Copy from nexus-share**: backend/lib/base-stack.ts (S3 bucket configuration)
    - Copy S3 bucket setup from nexus-share BaseStack
    - Keep versioning, CORS, and lifecycle policies
    - Export bucket name and ARN
    - _Requirements: 1.2_
  
  - [x] 2.4 Write unit tests for BaseStack
    - **Copy from nexus-share**: backend/test/base-stack.spec.ts or similar stack tests
    - Copy test structure and patterns from nexus-share CDK stack tests
...
- [x] 3. Implement GraphQL schema and API stack
  - [x] 3.1 Create GraphQL schema definition
    - **Copy from nexus-share**: backend/src/gql-schemas/ (User type, common patterns)
    - Copy User type definition from nexus-share schema (simplify by removing social-specific fields)
    - Copy or adapt Subscription type from nexus-share billing schema
    ...
    - Copy AI mutation patterns from nexus-share bedrock schema (enhanceContent, generateContent)
    - _Requirements: 2.1, 2.2, 2.3, 2.4, 17.12, 17.13, 17.14, 17.15, 17.16, 18.6, 18.7_
  
  - [x] 3.2 Create APIStack with AppSync GraphQL API
    - **Copy from nexus-share**: backend/lib/merged-api-stack.ts (AppSync API setup)
    - Copy AppSync API configuration from nexus-share merged-api-stack
...
- [x] 4. Implement repository layer for data access
  - [x] 4.1 Create UserRepository class
    - **Copy from nexus-share**: backend/src/repositories/ (repository pattern and DynamoDB client setup)
    - Copy repository base patterns from nexus-share repositories
    - Copy DynamoDB client initialization and error handling
    - Implement create method with DynamoDB PutItem (copy pattern from nexus-share)
    - Implement getById method with DynamoDB GetItem (copy pattern from nexus-share)
    - Implement update method with DynamoDB UpdateItem (copy pattern from nexus-share)
    - Implement delete method with DynamoDB DeleteItem (copy pattern from nexus-share)
    - Copy error handling patterns with descriptive messages from nexus-share
    - _Requirements: 4.1, 4.2, 4.3, 4.4, 4.5, 4.6_
  
  - [x] 4.2 Create SubscriptionRepository class
    - **Copy from nexus-share**: backend/src/repositories/ (repository patterns, especially any subscription/billing repository)
    - Copy repository patterns from nexus-share
    - Implement create method with DynamoDB PutItem (copy pattern)
    - Implement getByUserId method with DynamoDB Query on GSI (copy GSI query pattern from nexus-share)
    ...
    - Copy error handling patterns from nexus-share
    - _Requirements: 4.1, 4.2, 4.3, 4.4, 4.5, 4.6_
  ...
  - [x] 4.4 Write unit tests for UserRepository
    - **Copy from nexus-share**: backend/test/ (repository test patterns)
    - Copy test structure and mocking patterns from nexus-share repository tests
...
- [x] 6. Implement payment service with multi-provider support
  - [x] 6.1 Create PaymentService class with Stripe integration
    - **Copy from nexus-share**: backend/src/billing/ (Stripe integration patterns)
    - Copy Stripe client initialization from nexus-share
    - Copy Stripe subscription creation patterns from nexus-share billing service
    - Implement createStripeSubscription method (adapt from nexus-share)
    - Implement cancelStripeSubscription method (adapt from nexus-share)
    - Implement handleStripeWebhook method for payment events (copy webhook handling from nexus-share)
    - Copy error handling and status update patterns from nexus-share
...
- [x] 7. Implement subscription CDK stack
  - [x] 7.1 Create SubscriptionStack with Stripe webhook endpoint
    - **Copy from nexus-share**: backend/lib/subscription-stack.ts
    - Copy entire SubscriptionStack from nexus-share as it's already minimal
...
- [x] 8. Implement AI content enhancement service
  - [x] 8.1 Create AIService class with Bedrock integration
    - **Copy from nexus-share**: backend/src/bedrock/ (Bedrock service implementation)
    - Copy Bedrock client initialization from nexus-share
    - Copy enhanceContent method implementation from nexus-share
    - Copy generateContent method implementation from nexus-share
    - Copy rate limiting implementation from nexus-share (adapt to 10 requests per minute)
    - Copy CloudWatch logging patterns from nexus-share
    - Copy error handling patterns from nexus-share
    - _Requirements: 18.1, 18.2, 18.3, 18.11, 18.12_
  
  - [x] 8.2 Create BedrockStack with IAM permissions
    - **Copy from nexus-share**: backend/lib/bedrock-stack.ts
    - Copy entire BedrockStack from nexus-share
...
- [x] 9. Implement GraphQL resolvers
  - [x] 9.1 Create AppSync resolver functions for User operations
    - **Copy from nexus-share**: backend/src/appsyncFunctions/ (resolver patterns and utilities)
    - Copy AppSync resolver structure and patterns from nexus-share
    - Copy util.error() usage patterns from nexus-share resolvers
    - Implement getUser resolver (adapt from nexus-share user resolvers)
    - Implement createUser resolver (adapt from nexus-share user resolvers)
    - Implement updateUser resolver (adapt from nexus-share user resolvers)
  ...
  - [x] 9.3 Create AppSync resolver functions for AI operations
    - **Copy from nexus-share**: backend/src/appsyncFunctions/ (Bedrock/AI resolver patterns)
    - Copy AI resolver patterns from nexus-share
    - Implement enhanceContent resolver (copy from nexus-share bedrock resolvers)
    - Implement generateContent resolver (copy from nexus-share bedrock resolvers)
...
- [x] 10. Implement CloudFront distribution stack
  - [x] 10.1 Create DistributionStack with CloudFront configuration
    - **Copy from nexus-share**: backend/lib/distribution-stack.ts
    - Copy entire DistributionStack from nexus-share
...
- [x] 11. Implement build system for Lambda functions
  - [x] 11.1 Create esbuild configuration for AppSync functions
    - **Copy from nexus-share**: backend/build.mjs (entire build script)
    - Copy build.mjs script from nexus-share
...
- [x] 13. Initialize Flutter frontend project
  - [x] 13.1 Create Flutter project structure
    - **Copy from nexus-share**: frontend/src/ (entire Flutter project structure)
    - Copy Flutter project structure from nexus-share/frontend/src
    ...
    - Copy platform configurations (web, iOS, Android) from nexus-share
    - Copy assets directory structure
    - _Requirements: 5.1, 5.2, 5.3, 5.4, 5.5, 5.6, 13.2_
  
  - [x] 13.2 Configure Amplify integration
    - **Copy from nexus-share**: frontend/src/lib/amplifyconfiguration.dart and main.dart Amplify setup
    - Copy amplifyconfiguration.dart structure from nexus-share
    - Copy Amplify initialization code from nexus-share main.dart
...
- [x] 14. Implement Flutter authentication services
  - [x] 14.1 Create AmplifyAuthService class
    - **Copy from nexus-share**: frontend/src/lib/services/ (auth service implementation)
    - Copy auth service structure from nexus-share
    - Copy signIn method implementation from nexus-share
    - Copy signOut method implementation from nexus-share
    - Copy register method implementation from nexus-share
    - Copy getCurrentUser method implementation from nexus-share
    - Copy error handling patterns from nexus-share
    - _Requirements: 6.3, 6.4_
  
  - [x] 14.2 Create AuthProvider for state management
    - **Copy from nexus-share**: frontend/src/lib/providers/ (auth provider implementation)
    - Copy AuthProvider structure from nexus-share
    - Copy state management patterns (currentUser, isAuthenticated, isLoading)
    - Copy signIn, signOut, register method implementations
    - Copy listener notification patterns from nexus-share
    - _Requirements: 5.5_
...
- [x] 21. Create Angular join page application
  - [x] 21.1 Initialize Angular standalone project
    - **Copy from nexus-share**: join_page/ (entire Angular project structure)
    - Copy entire join_page directory from nexus-share as a starting point
    - Keep Angular 20+ standalone component architecture
    - Keep TypeScript configuration
    - Simplify content to be more generic (remove Nexus Share branding)
    - _Requirements: 7.1, 13.3_
  
  - [x] 21.2 Create HeroComponent
    - **Copy from nexus-share**: join_page/ (hero component if exists)
    - Copy or adapt hero section from nexus-share join page
    - Update value proposition to be generic
    - Keep responsive styling patterns from nexus-share
    - _Requirements: 7.2_
  
  - [x] 21.3 Create FeaturesComponent
    - **Copy from nexus-share**: join_page/ (features component if exists)
    - Copy or adapt features section from nexus-share join page
...
- [x] 22. Create Angular competitors page application
  - [x] 22.1 Initialize Angular standalone project
    - **Copy from nexus-share**: competitors_page/ if exists, or copy join_page structure
...
- [x] 23. Create Angular profile cards page application
  - [x] 23.1 Initialize Angular standalone project
    - **Copy from nexus-share**: profile-cards/ (entire Angular project)
    - Copy entire profile-cards directory from nexus-share
    - Keep Angular 20+ standalone component architecture
  ...
  - [x] 23.2 Create ProfileCardComponent
    - **Copy from nexus-share**: profile-cards/ (profile card component)
    - Copy ProfileCardComponent from nexus-share
  ...
  - [x] 23.3 Configure GraphQL API integration
    - **Copy from nexus-share**: profile-cards/ (GraphQL client setup)
    - Copy GraphQL client configuration from nexus-share profile-cards
...
- [x] 24. Implement deployment scripts and configuration
  - [x] 24.1 Create npm scripts for backend deployment
    - **Copy from nexus-share**: package.json scripts section
    - Copy deployment scripts from nexus-share package.json
  ...
  - [x] 24.2 Create npm scripts for testing and linting
    - **Copy from nexus-share**: package.json scripts section
    - Copy test, test:fast, lint, lint:fix, bt scripts from nexus-share
  ...
  - [x] 24.4 Create environment configuration files
    - **Copy from nexus-share**: cdk.context.json or cdk.json context section
    - Copy environment configuration structure from nexus-share
...
- [x] 25. Create documentation and README
  - [x] 25.1 Create root README.md
    - **Copy from nexus-share**: README.md structure
    - Copy README structure from nexus-share
    - Update project overview to describe minimal SaaS template
    - Copy setup instructions patterns from nexus-share
    - Copy common commands section from nexus-share
    - Copy environment configuration section from nexus-share
    - Copy deployment process section from nexus-share
    - Remove social network-specific documentation
    - _Requirements: 16.1, 16.2, 16.3, 16.4, 16.5_
  
  - [x] 25.2 Add inline code documentation
    - **Copy from nexus-share**: JSDoc and dartdoc comment patterns
    - Copy JSDoc comment patterns from nexus-share backend code
    - Copy dartdoc comment patterns from nexus-share frontend code
    - Copy complex logic comment patterns from nexus-share
    - _Requirements: 16.6, 16.7_

- [x] 26. Implement AWS SDK mocks for testing
  - [x] 26.1 Create mock implementations for AWS services
    - **Copy from nexus-share**: __mocks__/ and backend/test/mocks/
    - Copy AWS SDK mock setup from nexus-share __mocks__ directory
    - Copy Cognito client mocks from nexus-share
    - Copy DynamoDB client mocks from nexus-share
    - Copy Bedrock client mocks from nexus-share (if exists)
    - Copy S3 client mocks from nexus-share
    - Keep aws-sdk-client-mock library usage patterns
    - _Requirements: 11.3_
  
  - [x] 26.2 Create test fixtures
    - **Copy from nexus-share**: backend/test/fixtures.ts and backend/test/mocks/
    - Copy test fixtures structure from nexus-share
    - Copy user fixtures from nexus-share (simplify by removing social fields)
    - Copy or create subscription fixtures for all providers
    - Copy payment event fixtures from nexus-share billing tests
    - Copy AI request/response fixtures from nexus-share bedrock tests
    - _Requirements: 11.4_

- [x] 27. Implement error handling and logging
  - [x] 27.1 Add CloudWatch logging to Lambda functions
    - **Copy from nexus-share**: backend/src/ (logging patterns from any Lambda function)
    - Copy structured JSON logging patterns from nexus-share Lambda functions
    - Copy correlation ID implementation from nexus-share
    - Copy error logging with stack traces patterns
    - Copy AI usage logging patterns from nexus-share bedrock functions
    - _Requirements: 18.12_
  
  - [x] 27.2 Add error handling to all resolvers
    - **Copy from nexus-share**: backend/src/appsyncFunctions/ (error handling patterns)
    - Copy util.error() usage patterns from nexus-share resolvers
    - Copy error type mapping from nexus-share (Unauthorized, NotFound, ValidationError, InternalError)
    - Copy descriptive message patterns from nexus-share
    - Keep stack trace hiding patterns from nexus-share
    - _Requirements: 2.6, 3.7, 4.6_
  
  - [x] 27.3 Add error handling to Flutter services
    - **Copy from nexus-share**: frontend/src/lib/services/ (error handling patterns)
    - Copy GraphQL error mapping from nexus-share services
    - Copy retry logic with exponential backoff from nexus-share
    - Copy network connectivity handling from nexus-share
    - Copy error state display patterns from nexus-share widgets
    - _Requirements: 6.6_
...
- **IMPORTANT**: Maximize code reuse from nexus-share - copy entire files/directories where possible and simplify by removing social media features
- Each task now includes specific "Copy from nexus-share" instructions indicating what to copy
